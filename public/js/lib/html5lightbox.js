...
inst.$close.click(function(){
                	...
                	$('.mountain_list ul').removeClass("selected-border");
	$('.mountain_list ul').find("div span span div.mountaintickmark").css("display", "none");
	selectedFeedArr.length = 0;
$("#m_push").attr("src","public/images/push_n.png");
                });
...
//alert(inst.$text.text().length);
            ...
            inst.embedHTML5Video = function($container,
                src, autoplay) {
                $container.html("<div style='display:block;width:100%;height:100%;position:relative;'><video id='html5-lightbox-video' width='100%' height='100%'" + (autoplay ? " autoplay" : "") + (inst.options.nativehtml5controls && !inst.options.videohidecontrols ? " controls='controls'" : "") + " src='" + src + "'></div><div id='overlay' style='display:none'><img id='loading' src='/public/images/32.gif'></div>");
